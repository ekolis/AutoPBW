...
namespace AutoPBW
{
	...
	public class SEPBW
		: IMultiplayerService
	{
		public string ServiceTypeName { get; } = "Space Empires PBW";

		public string Name { get; set; } = "Space Empires PBW";
		...
		public bool UploadHostTurn(HostGame game, IEnumerable<string> files)
		{
			try
			{
				var url = $"http://pbw.spaceempires.net/games/{game.Code}/host-turn/upload";
				game.ArchiveUploadAndDeleteArchive(files, url, "turn_file", HttpStatusCode.Redirect); // for some reason PBW gives a 302 on host turn upload
				return true;
			}
			catch
			{
